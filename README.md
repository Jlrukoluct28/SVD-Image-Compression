...
2. SVD применяется к каждой цветовой матрице по формуле: $A = U \Sigma V^T$.
3. Полученные матрицы усекаются до первых $k$ компонент (формируется низкоранговая аппроксимация): $A_k = U_k \Sigma_k V_k^T$.
4. Сжатые каналы собираются обратно в единый RGB-тензор и отрисовываются через matplotlib.
